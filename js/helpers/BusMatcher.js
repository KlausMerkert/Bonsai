...
BusMatcher.prototype.matchBitRegisters = function () {
    for (var i = 0; i < this.cpu.bitregisters.length; i++) {
        ...
        if (this.cpu.bitregisters[i].widegate.writeWireId) {
            this.cpu.bitregisters[i].widegate.writeWire = this.findBus(this.cpu.bitregisters[i].widegate.writeWireId);
        }
        if (this.cpu.bitregisters[i].widegate.readWireId) {
            this.cpu.bitregisters[i].widegate.readWire = this.findBus(this.cpu.bitregisters[i].widegate.readWireId);
        }
        if (this.cpu.bitregisters[i].wiresReadWireId) {
            this.cpu.bitregisters[i].wiresReadWire = this.findBus(this.cpu.bitregisters[i].wiresReadWireId);
        }
        if (this.cpu.bitregisters[i].wiresWriteWireId) {
            this.cpu.bitregisters[i].wiresWriteWire = this.findBus(this.cpu.bitregisters[i].wiresWriteWireId);
        }
        for (var j = 0; j < this.cpu.bitregisters[i].wiregates.length; j++) {
            this.cpu.bitregisters[i].wiregates[j].wire = this.findBus(this.cpu.bitregisters[i].wiregates[j].wireId);
        }
    }
};
